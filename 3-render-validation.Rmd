...
mediation$sentinel <- factor(mediation$sentinel, levels=unique(mediation$sentinel))

# plot the mediation results
toplot <- melt(mediation, measure.vars=c(3,4), value.name = "pval")
...
# sort by log_fc
mediation <- mediation[order(mediation$log10_med, decreasing = T),]
mediation$sentinel <- factor(mediation$sentinel, levels=unique(mediation$sentinel))

toplot <- melt(mediation, measure.vars=5, value.name="log_fc")
...
genes ($p_s$). The red bars (*selected*) indicate fold changes where $p_n$ is lower 
...
(i.e. $p_s$ being smaller than their respective $p_n$s).
