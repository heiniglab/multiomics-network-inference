...
fits <- lapply(cohorts, function(c){
  fit <- tryCatch({
    ...
    fit.out <- paste0("results/current/fits/", sentinel, ".", c, ".RData")
    if(!file.exists(fit.out)){
      ...
      if(length(nodes(graph))<1){
        ...
        plot.data <- plot.ggm(g=graph, id=id, dot.out = paste0("results/current/fits/", sentinel, ".", c, ".dot"))
        save(file=fit.out, 
  ...
  #try-catch error
  error=function(m){
    f <- paste0("results/current/fits/", sentinel, ".out")
...
if(!any(is.null(unlist(fits)))) {
  ...
  save(file=paste0("results/current/fits/", sentinel, ".RData"), fits)
}
```
