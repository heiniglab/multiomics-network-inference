...
library(ggplot2)
library(wesanderson)
pal_cols <- wes_palette(n=4, "Moonrise2")
```
...
gg <- geom_violin(draw_quantiles=c(.25,.5,.75), position = position_dodge(0.7) )
...
ggplot(tab, aes(y=specificity,x=R,fill=comparison)) +
	gg + sfm +
	ggtitle("Specificity", subtitle = paste0("(N = ", N, ")"))

ggplot(tab, aes(y=sensitivity,x=R,fill=comparison)) + 
	gg + sfm + 
	ggtitle("Sensitivity", subtitle = paste0("(N = ", N, ")"))
