...
bdgraph = function( data, n = NULL, method = "ggm", algorithm = "bdmcmc",
					...
					multi.update = NULL, save.all = FALSE , link.priors = NULL)
{
	threshold = 1e-8  # for sampling from gwishart distribution
	burnin    = floor( burnin )

	# TODO: neately implement handling of link priors, remove this stop
	if(is.null(link.priors)) stop("Link priors must be provided for now.");

	# check link priors, must not be 0 or 1
	if(any(link.priors == 0) | any(link.priors == 1)) {
	  stop("Link priors must not be 0 or 1.");
	}
...
#~ 			data = data / sd( data[ , 1] )
	...
	cat("Using", paste0(method, algorithm, paste0("mu",multi_update), collapse="_"), "\n");
	...
	if( save.all == TRUE )
	{
		if( ( method == "ggm" ) && ( algorithm == "bdmcmc" ) && ( multi_update == 1 ) )
		{
			result = .C( "ggm_bdmcmc_map", as.integer(iter), as.integer(burnin), G = as.integer(G), as.double(Ts), K = as.double(K), as.integer(p),
						...
						as.integer(b), as.integer(b_star), as.double(Ds), as.double(threshold), P=as.double(link.priors), PACKAGE = "BDgraph" )
		}
		...
		if( ( method == "ggm" ) && ( algorithm == "bdmcmc" ) && ( multi_update != 1 ) )
		{
			result = .C( "ggm_bdmcmc_map_multi_update", as.integer(iter), as.integer(burnin), G = as.integer(G), as.double(Ts), K = as.double(K), as.integer(p),
						all_graphs = as.integer(all_graphs), all_weights = as.double(all_weights), K_hat = as.double(K_hat),
						sample_graphs = as.character(sample_graphs), graph_weights = as.double(graph_weights), size_sample_g = as.integer(size_sample_g),
						as.integer(b), as.integer(b_star), as.double(Ds), as.double(threshold), as.integer(multi_update), P=as.double(link.priors), PACKAGE = "BDgraph" )
		}
		...
		if( ( method == "gcgm" ) && ( algorithm == "bdmcmc" ) && ( multi_update == 1 ) )
		{
			result = .C( "gcgm_bdmcmc_map", as.integer(iter), as.integer(burnin), G = as.integer(G), as.double(Ts), K = as.double(K), as.integer(p),
						...
						all_graphs = as.integer(all_graphs), all_weights = as.double(all_weights), K_hat = as.double(K_hat),
						sample_graphs = as.character(sample_graphs), graph_weights = as.double(graph_weights), size_sample_g = as.integer(size_sample_g),
						as.integer(b), as.integer(b_star), as.double(D), as.double(Ds), as.double(threshold), P=as.double(link.priors), PACKAGE = "BDgraph" )
		}
		...
		if( ( method == "gcgm" ) && ( algorithm == "bdmcmc" ) && ( multi_update != 1 ) )
		{
			result = .C( "gcgm_bdmcmc_map_multi_update", as.integer(iter), as.integer(burnin), G = as.integer(G), as.double(Ts), K = as.double(K), as.integer(p),
						...
						all_graphs = as.integer(all_graphs), all_weights = as.double(all_weights), K_hat = as.double(K_hat),
						sample_graphs = as.character(sample_graphs), graph_weights = as.double(graph_weights), size_sample_g = as.integer(size_sample_g),
						as.integer(b), as.integer(b_star), as.double(D), as.double(Ds), as.double(threshold), as.integer(multi_update), P=as.double(link.priors), PACKAGE = "BDgraph" )
		}
