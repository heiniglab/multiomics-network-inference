...
tab <- read.table("results/current/validation.summary_nopriors.txt", header=T, sep="\t")[1:26]
head(tab)
# create a mediation specific plot
# TODO update those indices
mediation <- tab[,c("sentinel", "cohort", 
...
#### Non-selected genes with strong mediation effect
Apparently, some of the results above indicate that there are genes, which show
a very strong effect mediation but are not selected by our model. Here we briefly
analyse those genes to get an idea of what went wrong.

```{r}
source("R/validation.R")

# check the most extreme sentinel here
snp <- "rs3809627"
#snp <- "rs3764289"
# load preprocessed cohort data
env <- new.env()
load("results/current/data.processed.RData", envir=env)
data <- with(env, data[[snp]]$lolipop$data)
cohort <- "kora"
load(paste0("results/current/fits_nopriors/", snp, ".", cohort, ".RData"))
...
med <- mediation(data, snp, sgenes, cpgs, plot=T)
# build plotting data.frame
temp <- cbind.data.frame(gene=names(med), pvalue=unlist(lapply(med, "[", "pvalue")))
temp$gene <- as.character(temp$gene)
temp <- cbind.data.frame(temp, selected=(temp$gene %in% sgenes.selected))
temp <- cbind.data.frame(temp, snp.prior=gpriors[temp$gene,snp], in.gstart=gstart[temp$gene,snp])

# get the linear model fits between genes and our snp
cors <- lapply(temp$gene, function(g){
  mod <- lm(paste0("`", g, "`~", snp), data=data)
  lmp(mod)
})
names(cors) <- temp$gene
temp <- cbind.data.frame(temp, lmp=unlist(cors)[temp$gene])
# show only data for entities where a prior was assigned
temp <- temp[temp$snp.prior>min(temp$snp.prior),]
temp$snp.prior

# create to summary plots
ggplot(aes(y=snp.prior, x=selected, fill=selected), data = temp) + geom_boxplot()
ggplot(aes(y=-log10(lmp), x=-log10(pvalue),  col=snp.prior, shape=selected, size=2), data = temp) + geom_point()

# interesting gene to follow up prior/selected/mediation associations
med[["TBC1D10B"]]
...
results <- tab[,c("snp_genes", "snp_genes_selected",
...
sgenes.selected <- results["snp_genes_selected"]
sgenes.notselected <- results["snp_genes"] - sgenes.selected
# proportion of significant genes per group
sign.selected <- results["mediation_selected_significant"] / sgenes.selected
sign.notselected <- results["mediation_significant"] / sgenes.notselected
...
gg <- tab[,c("sentinel","cohort","geuvadis_gene_gene","geo_gene_gene","cohort_gene_gene")]
