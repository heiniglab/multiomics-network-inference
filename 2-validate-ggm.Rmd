...
pvs <- lapply(1:nrow(em), function(r){
  ...
  if(n1 %in% colnames(data) & n2 %in% colnames(data)){
    pv <- cor.test(data[,n1], data[,n2])$p.value
    c(n1,n2,pv)
  } else {
    c(n1,n2,NA)
  }
})
pvs <- as.data.frame(matrix(unlist(pvs), ncol=3, byrow = T), stringsAsFactors=F)
colnames(pvs) <- c("n1", "n2", "pval")
...
sign.cors <- pvs[abs(pvs$qval)<0.01,]
